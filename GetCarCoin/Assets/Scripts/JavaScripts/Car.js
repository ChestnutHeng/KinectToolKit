...
var count : int = 0;
public static var coinnum :int= 0;
...
{	
    ...
    GameObject.Find("Coins").guiText.text = "Coins: " + coinnum;
    GameObject.Find("Cards").guiText.text = "Cards:  " + Application.loadedLevel;
    GameObject.Find("Rank").guiText.text = ("Rank: " + rank);
    GameObject.Find("Timer").guiText.text = ("Time: " + Time.time); 
}
...
function OnCollisionEnter(obj:Collision)
{
	...
	if(obj.gameObject.CompareTag("Door") && (coinnum == 2))
	{
	 	Application.LoadLevel("Stage2");
	 	rank = rank + 200;
	    bonus = (10-Time.time)*20;
	 	if (bonus > 0) rank = rank + bonus;
	 	
	}
    else if (obj.gameObject.CompareTag("Door2") && (coinnum == 4))
    {
      	Application.LoadLevel("Stage3");
      	rank = rank + 200;
      	bonus = (30-Time.time)*20;
	 	if (bonus > 0) rank = rank + bonus;
    }
    else if(obj.gameObject.CompareTag("Door3") && (coinnum == 8))
    {
        rank = rank + 200;
        bonus = (120-Time.time)*20;
	 	if (bonus > 0) rank = rank + bonus;
        GameObject.Find("Win").guiText.text = "You win!";
        Application.Quit();
    }
}